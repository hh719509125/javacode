...
public class CreateDirAndFileAppMain{
    ...
    private static void deleteDir(File newDir){
        ...
        // TODO 删除文件夹的前提是文件夹是空的，否则会删除失败
        if (deleteDir){
            System.out.print("删除文件夹：" + newDir.delete());
        }
    }
    ...
    private static File renameDir(File dir){
        ...
        if (renameSuccess) {
            System.out.print("改名为" + newDirName + "成功");
        }else {
            System.out.print("改名为" + newDirName + "失败");
            return null;
        }
        return newDir;
    }
    ...
    private static File createDirs()  {
        ...
        while (true){
            System.out.print("请输入文件路径，如果为空则结束");
    ...
    private static File createDir(String... restPaths)  {
        ...
        System.out.print("将在" + ROOT + "下创建" + rest);
        File dir = new File(ROOT, rest);
        if (dir.exists() && dir.isDirectory()){
            System.out.print("文件夹已存在" + dir.toString());
            return dir;
        }else {
