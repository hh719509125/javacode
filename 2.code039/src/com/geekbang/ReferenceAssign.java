...
//        phone notDoable = new MerchandiseV2();
        ...
        // >> TODO                           重点
        // >> TODO 因为子类继承了父类的方法和属性，所以父类的对象能做到，子类的对象肯定能做到
        //    TODO 换句话说，我们可以在子类的对象上，执行父类的方法
        // >> TODO 当父类的引用指向子类的实例（或者父类的实例），只能通过父类的引用，像父类一样操作子类的对象
        //    TODO 也就是说“名”的类型，决定了能执行哪些操作


        // >> TODO ph和m都指向同一个对象，通过m就不可以调用getBrand方法
        //    TODO 因为ph的类型Phone，Phone定义了getBrand方法
        ph.getBrand();
        // >> TODO ph和m都指向同一个对象，但是通过m就不能调用getBrand方法
        //    TODO 因为m类型是MerchandiseV2，MerchandiseV2里没有定义getBrand方法
//        m.getBrand();
        ...
        // TODO 父类的引用，可以指向子类的对象，既可以用子类（以及子类的子类）的引用给父类的引用赋值
        MerchandiseV2 ccm = shellColorChangePhone;

        // TODO 父类的引用，可以指向子类的对象
        // TODO 确定MerchandiseV2的引用ccm是指向的Phone或者Phone的子类对象，那么可以强制类型转换
        Phone ccp = (Phone) ccm;

        // TODO 确定MerchandiseV2的引用ccm是指向的ShellColorChangePhone的对象
        // TODO 那么可以强制类型转换
