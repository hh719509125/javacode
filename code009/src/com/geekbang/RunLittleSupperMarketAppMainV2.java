...
public class RunLittleSupperMarketAppMainV2 {
    public static void main(String[] args) {
        ...
        //为了方便，创建一个商品数组的引用，和littleSuperMarket.merchandises指向同一个数组对象
        ...
        for (int i = 0; i < all.length; i++){
            ...
            m.id = "ID" + i;
            m.gift = giftNoodle;
            //用创建的商品，给商品数组第i个引用赋值，all和小超市的商品数组引用指向的是同一个数组对象
            all[i] = m;
            m.describe();
        }
        ...
        //参数
        // >> TODO 参数的传递，其实就是赋值。左边的是形参，右边是括号里的实参
        ...
        m.willOutsideValueChangeIfParameterValueChangeHerePef(paramRef);
        //效果类似如下代码
        ...
        // >> TODO 可以把返回值的方法，当成一个成员变量，当成一个类型为返回值类型的成员变量
        //    TODO 关于返回值并忽略方法执行的部分，使用返回值，就好像在使用一个成员变量
