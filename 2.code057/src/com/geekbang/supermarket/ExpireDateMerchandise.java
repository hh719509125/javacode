...
// >> TODO 同样接口不可以声明实例变量。其他方法是有限制的，比如这个接口，因为不能声明实例变量，
// >> TODO 只能通过getProducedDate和getExpireDate,间接通过实现接口的类，获取数据
// >> TODO 和抽象的方法不同，如果一个类实现了两个接口，并且两个接口里有相同的缺省方法，编译器会报错
public interface ExpireDateMerchandise {

    // >> TODO 缺省方法，也有this自引用，但是只能调用接口里的方法，或者继承接口里的方法
    // >> TODO 因未能new出实例来的，肯定是实现了所有方法的，this子引用就是指向了那个对象，所以使用起来不会有问题

    /**
     * 截止到当前，商品的保质期天数是否超过了传递的天数
     *
