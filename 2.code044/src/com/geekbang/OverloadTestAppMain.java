...
public class OverloadTestAppMain {
    public static void main(String[] args) {
        ...
        // TODO 重载调用那个方法，和参数的引用类型相关，和引用实际指向的类型无关
        ...
        merchandiseTest.tesMerchandiseOverload((String) null);

        System.out.println("------------------2------------------");

        System.out.println();
        // >> TODO 引用本身是null没关系，确定调用那个方法只需要引用的类型。这叫做静态多态。即在编译期就知道该调用哪个方法
        m = null;
        merchandiseTest.tesMerchandiseOverload(m);
        merchandiseTest.tesMerchandiseOverload((Phone) m);
        merchandiseTest.tesMerchandiseOverload((ShellColorChangePhone) m);
        ...
        merchandiseTest.tesMerchandiseOverload((ShellColorChangePhone) null);


        // >> TODO 重载总结：静态多态，调用的方法和参数实际指向的对象无关，只和引用本身的类型相关
        // >> TODO 因为调用时参数类型是确定的，所以，在编译期间就可以明确的知道哪个方法会被调用。如果有多种可能，则会有编译错误
        // >> TODO 如果没有类型完全匹配的候选，则根据类型的继承关系向下撸着找。找到了最贴近参数类型的那个方法
