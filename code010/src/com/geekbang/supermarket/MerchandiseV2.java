...
public class MerchandiseV2 {
    ...
    public void describe() {
        System.out.println("商品的名字叫做" + name + "，id是" + id + "。商品售价是" + soldPrice
                + "。商品的进价是" + purchasePrice + "商品的赠品是" + gift.name + "。价值" + gift.purchasePrice);
    }
    ...
    // >> TODO 参数定义的规范和变量一样，都是类型名字加标识符，这里的表识符我们叫做参数名。
    // >> TODO 方法体中的代码可以使用参数
    ...
    // >> TODO 参数可以是任何类型，包括自定义类型，甚至是自己类型都没问题
    public boolean isTheBiggestTotalValueOne(LittleSuperMarket littleSuperMarket){
        ...
        for (int i = 0;i < littleSuperMarket.merchandises.length;i++){
            ...
            if (totalValue < newTotalValue){
                // >> TODO 执行到return的时候，方法直接结束，不管是不是再循环中，是在第几层中。
                return false;
            }
        }
        return true;
    }
    //---------------------------分清参数、局部变量和实例变量的底盘 新增内容----------------------------
    ...
    public void willOutsideValueChangeIfParameterValueChangeHerePef(MerchandiseV2 m2){
        m2 = gift;
    }
