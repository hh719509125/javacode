...
public class WrapperClassesForNumberPrimaryTypesAppMain {
    public static void main(String[] args) {
        ...
        // >> TODO 和数字相关的数据类型对应的类依次为：Byte，Short，Integer，long，Float，Double
        int a = 99;
        // >> TODO 可以使用数字创建一个Integer类，下面的方法在别的数字类中也都相似
        ...
        // >> TODO 从基本数据类型到封装类，我们叫做自动封箱，英文名叫做auto boxing，反之叫做自动拆箱，叫做auto unboxing
        Integer ab = 987;
        int cd = ab;
        ...
        int2Str.put(2, "贰");
        int2Str.put(3, "叁");
        System.out.print(int2Str.get(1));

        // TODO 自动拆箱为 int ，并给key赋值
        for (int key : int2Str.keySet()){
            System.out.print(key);
        }

        System.out.print("---------------有用的方法---------------");

        System.out.print(Integer.toBinaryString(-1024));
        System.out.print(Integer.toOctalString(-1024));
        System.out.print(Integer.toHexString(-1024));

        System.out.print("----------------Number类-----------------");

        // TODO 所有和数字相关的封装类，都继承值自Number类
        Number num = 9;
        ...
        System.out.print("使用number将double转为long：" + numD.longValue());


        System.out.print("-----------------自动拆箱可能的NPE-----------------");

        int2Str.put(null,"无");
        System.out.print(int2Str.get(null));

        // TODO 自动拆箱为int，并给key赋值，但是有一个key为null，null是无法转成一个 int 的，注意，null 不是 0，
        // TODO 其实自动拆箱，后面是Java帮我们调用了对应的方法，在这里就是intValue()这个方法
        // TODO 所以当引用为null的时候自动拆箱，相当于是调用null方法，所以这时候就会发生NPE
