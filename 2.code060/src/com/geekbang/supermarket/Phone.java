...
public class Phone extends MerchandiseV2 {
    ...
    // >> TODO 接口也可以定义为静态内部接口，但一般不这么做。接口的目的是为了让更多人实现，所以一般会是单独一个文件作为公共接口
...
//        localCPUHZ = Math.random();
        ...
        // >> TODO 局部内部类，不可以有访问控制符。局部内部和成员方法，成员变量一样，都是类的组成部分
        class CPU implements UnitSpec {
            ...
            @Override
            public double getNumSpec() {
                // >> TODO 局部内部类，代码和这个类本身访问权限一样，可以访问外部 （Phone） 的private属性
                // >> TODO 局部内部类中有一个外部类的引用
                // >> TODO 局部内部类访问外部类的对象的成员属性的完整写法如下，类名.this.属性/方法
                // >> TODO 以上都和成员内部类一样。除此之外，局部内部类还可以访问参数和局部变量，但是他两必须是实际final的
                // 仅做访问数据的演示，没有实际意义
            ...
            public void getProducer(String producer) {
                this.producer = producer;
            }
...
//            }
    ...
    public void getStorageG(int storageG) {
        this.storageG = storageG;
    }
